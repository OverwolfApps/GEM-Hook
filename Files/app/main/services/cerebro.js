(function() {
	...
	angular
		.module('gemApp')
		.service('cerebro', cerebro);

	function cerebro($rootScope, $q, $localStorage, $sessionStorage, $timeout, messenger) {
		...
		function newEventReceived(result) {
			if (!gameInfo) return console.warn('Event received but gameInfo is not present.');
			var currentGameID = parseInt(gameInfo.id / 10);
			...
			result.events.forEach(function(event) {
				...
				if (event.data) {
					try {
						//if (currentGameID === 10798 && (event.name === 'goal' || event.name === 'score')) event.data = decodeURI(event.data);
				...
				hooks.forEach(function(hook) {
					if (!hook.enabled) return console.log('Hook disabled, returning.', hook);
					...
					switch(hook.selectedAction) {
						...
						case 'take screenshot':
							q(overwolf.media.takeScreenshot)
								.then(function(result) {
									...
									if (hook.screenshot.share) q(overwolf.media.shareImage, result.url, '');
								});
							break;
						case 'save replay':
							overwolf.media.replays.capture(Math.abs(hook.replay.pastDuration) * 1000, hook.replay.futureDuration * 1000,
								function(result) {
									//finished callback
								...
								function(result) {
									//status callback
									if (result.error) captureError(result);
								}
							);
							break;
						case 'show image':
							messenger.send('notificationWindow', 'showImage', hook.image);
							break;
						...
						case 'send keystrokes':
							...
							break;
						case 'open url':
							console.log('browser', hook.openURL.default);
							if (hook.openURL.default) {
								overwolf.utils.openUrlInDefaultBrowser(hook.openURL.url);
							} else {
								overwolf.utils.openUrlInOverwolfBrowser(hook.openURL.url);
							}
							break;
						case 'set device lighting':
							...
							overwolf.logitech.led.saveCurrentLighting(function() {});
							...
							if (hook.keyboard.ignoreDuration) {
								processingLighting = false;
								return;
							}
							$timeout(function() {
								overwolf.logitech.led.restoreLighting(function() {});
								processingLighting = false;
							}, duration);
							break;
					...
					if (features.sort().join(', ') !== lastRequiredFeatures.sort().join(', ')) {
						...
						q(overwolf.games.events.setRequiredFeatures, features)
							...
							.catch(function(result) {
								...
								$timeout(updateRequiredFeatures, 5000);
							});
					}
					break;
			}
			...
			$localStorage.hooks.forEach(function(hook) {
				switch (hook.selectedAction) {
					case 'play sound':
						...
						break;
					case 'show image':
						if (preloadedResources.indexOf(hook.image.url.toLowerCase()) === -1) {
							$timeout(function() {
								messenger.send('notificationWindow', 'preloadImage', hook.image);
							}, 1000);
						}
						break;
				}
		...
		function messageReceived(event, message) {
			switch (message.id) {
				case 'imagePreloaded':
					...
					preloadedResources.push(content.url.toLowerCase());
					console.log('Image preloaded.', content.url);
					break;
			}
		}

		function gameInfoUpdated(result) {
			if (result.runningChanged) gameInfo = null;
			else gameInfo = result.gameInfo;

			if (result.gameChanged) {

				lastRequiredFeatures = [];
		...
		function runningGameInfo(gameInfo) {
			gameInfo = gameInfo;
		}
		...
		function playSound(url) {
			overwolf.media.audio.create(url, function(result) {
				if (result.status === 'success' || result.id) overwolf.media.audio.play(result.id, function(result) { console.log('playSound result', result); });
			});
		}
		...
		function setKeystrokes(keystrokes, index) {
			...
			if (index + 1 < keystrokes.length) $timeout(setKeystrokes.bind(null, keystrokes, index + 1), keystroke.delay || 1);
		...
		function setMacroState(macro, index, data) {
			...
			Promise.all(promises)
				.then(function() {
					if (index + 1 < macro.states.length) $timeout(setMacroState.bind(null, macro, index + 1, data), duration);
			...
			q(overwolf.windows.obtainDeclaredWindow, 'playerWindow')
				.then(function(result) {
					if (result.window.isVisible) {
						messenger.send('playerWindow', 'videoURL', {url: url});
					} else {
						q(overwolf.windows.restore, result.window.id)
							.then(function() {
								$timeout(messenger.send.bind(null, 'playerWindow', 'videoURL', {url: url}), 500);
							});
					}
				});
		}

		function instantReplayHandler(result) {
		...
		function eventError(result) {
			messenger.send('notificationWindow', 'showNotification', {
				type: 'error',
				text: result.error || result
			});
		}
		...
		overwolf.games.events.onError.addListener(eventError);

		overwolf.media.replays.onCaptureError.addListener(captureError);

		overwolf.games.onGameInfoUpdated.addListener(gameInfoUpdated);

		overwolf.games.getRunningGameInfo(runningGameInfo);

		overwolf.settings.registerHotKey('instant-replay', instantReplayHandler);
